...
(function (w) {
  ...
  var ALIAS = {
    ...
    "mykonos island": "mykonos", "jmk": "mykonos", "lgmk": "mykonos",
  ...
  /* Airport codes that arrive with no city attached, and the city each one is.
     Two sources leave bare codes: ProAir list IATA codes as the airport name,
     and Jet OUT tiles sometimes carry only an identifier. The scrape rule is not
     to invent a city, which is right, so the card used to read "MAD to BCN".
     This is the lookup that was missing, and it holds only codes whose airport
     is unambiguous. Anything not in here keeps showing its code, which is still
     better than a guess.

     Added 2 September 2026 after Radu saw MAD and BCN on the Barcelona page. */
  var CODE = {
    // IATA, from ProAir Aviation
    "MAD": "Madrid", "BCN": "Barcelona", "DUS": "Dusseldorf",
    "FMO": "Munster", "HAJ": "Hannover", "HAM": "Hamburg", "TXL": "Berlin",
    "STR": "Stuttgart", "NUE": "Nuremberg", "LEJ": "Leipzig", "BRE": "Bremen",
    "CGN": "Cologne", "FRA": "Frankfurt", "MUC": "Munich", "ZRH": "Zurich",
    "GVA": "Geneva", "VIE": "Vienna", "LIS": "Lisbon", "OPO": "Porto",
    "AGP": "Malaga", "PMI": "Palma", "IBZ": "Ibiza", "NCE": "Nice",
    "CDG": "Paris", "LBG": "Paris", "ORY": "Paris", "AMS": "Amsterdam",
    "BRU": "Brussels", "MXP": "Milan", "LIN": "Milan", "FCO": "Rome",
    "ATH": "Athens", "IST": "Istanbul", "CPH": "Copenhagen", "ARN": "Stockholm",
    "OSL": "Oslo", "HEL": "Helsinki", "DUB": "Dublin", "EDI": "Edinburgh",
    // ICAO, from Jet OUT tiles that carry only an identifier
    "KCRQ": "Carlsbad", "KHXD": "Hilton Head", "KVTN": "Valentine",
    "MMSD": "San Jose del Cabo", "MMPR": "Puerto Vallarta", "MMSL": "Cabo San Lucas",
    "MYEH": "North Eleuthera", "MBPV": "Providenciales", "TAPA": "Antigua"
  };

  /* The name to print on a card. Only bare codes are translated: a real name is
     left exactly as the operator wrote it. */
  function codeCity(text) {
    var t = String(text || "").trim().toUpperCase();
    return /^[A-Z0-9]{3,4}$/.test(t) ? (CODE[t] || "") : "";
  }

  w.EL_PLACE = { match: match, slug: slug, countryOf: countryOf, countriesPresent: countriesPresent, COUNTRY: COUNTRY, codeCity: codeCity };
})(window);
