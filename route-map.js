  ...
  function draw(host) {
    ...
    if (cap && cap.tagName === "P") {
      ...
      cap.style.marginBottom = "";
        ...
        /* Touch: a one finger drag must scroll the page, not pan the map. Leaflet's
       default grabs the gesture, so on a phone the map swallows the scroll and the
       page appears stuck. dragging is switched off on coarse pointers and tap is
       disabled with it; pinch to zoom still works, and the zoom buttons still work.
       Reported from a real phone on 27 Aug 2026. */
    var coarse = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;
    var map = L.map(wrap, { zoomControl: true, scrollWheelZoom: false,
                            dragging: !coarse, tap: false, attributionControl: true });
